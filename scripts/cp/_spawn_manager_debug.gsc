	...
	/#
		if(GetDvarString("Dev Block strings are not supported") == "Dev Block strings are not supported")
		{
			if(!isdefined(level.spawn_manager_debug_hud_title))
			{
				...
				level.spawn_manager_debug_hud_title.alignX = "Dev Block strings are not supported";
			...
			level.spawn_manager_debug_hud_title setText("Dev Block strings are not supported" + spawn_ai + "Dev Block strings are not supported" + active_ai + "Dev Block strings are not supported" + potential_ai + "Dev Block strings are not supported" + MAX_AI + "Dev Block strings are not supported" + active_managers);
			for(i = 0; i < level.debugActiveManagers.size; i++)
			{
				if(!isdefined(level.spawn_manager_debug_hud[i]))
				{
					...
					level.spawn_manager_debug_hud[i].alignX = "Dev Block strings are not supported";
				...
				text = "Dev Block strings are not supported" + level.debugActiveManagers[i].sm_id + "Dev Block strings are not supported";
				text = text + "Dev Block strings are not supported" + level.debugActiveManagers[i].spawncount;
				text = text + "Dev Block strings are not supported" + level.debugActiveManagers[i].activeAI.size + "Dev Block strings are not supported" + level.debugActiveManagers[i].sm_active_count_min + "Dev Block strings are not supported" + level.debugActiveManagers[i].sm_active_count_max + "Dev Block strings are not supported";
				text = text + "Dev Block strings are not supported" + level.debugActiveManagers[i].allSpawners.size;
				if(isdefined(level.debugActiveManagers[i].sm_group_size))
				{
					text = text + "Dev Block strings are not supported" + level.debugActiveManagers[i].sm_group_size + "Dev Block strings are not supported" + level.debugActiveManagers[i].sm_group_size_min + "Dev Block strings are not supported" + level.debugActiveManagers[i].sm_group_size_max + "Dev Block strings are not supported";
				}
	...
	/#
		...
		while(1)
		{
			...
			if(isdefined(level.debugActiveManagers) && level.debugActiveManagers.size > 0)
			{
				...
				if(level.test_player buttonpressed("Dev Block strings are not supported"))
				{
				...
				if(isdefined(current_spawn_manager_index) && current_spawn_manager_index != -1)
				{
					...
					else if(isdefined(level.debugActiveManagers[current_spawn_manager_index]))
					{
				...
				if(isdefined(spawn_manager.spawners))
				{
					for(i = 0; i < spawn_manager.spawners.size; i++)
					{
						...
						if(isdefined(current_spawner) && current_spawner.count > 0)
						{
							...
							print3d(current_spawner.origin + VectorScale((0, 0, 1), 65), "Dev Block strings are not supported" + current_spawner.count, (0, 1, 0), 1, 1.25, 2);
							print3d(current_spawner.origin + VectorScale((0, 0, 1), 85), "Dev Block strings are not supported" + current_spawner.activeAI.size + "Dev Block strings are not supported" + spawnerFree + "Dev Block strings are not supported" + current_spawner.sm_active_count, (0, 1, 0), 1, 1.25, 2);
						}
					}
				}
				wait(0.05);
...
function spawn_manager_debug_spawn_manager_values_dpad()
{
	/#
		...
		if(!isdefined(level.spawn_manager_debug_hud2))
		{
			...
			level.spawn_manager_debug_hud2.alignX = "Dev Block strings are not supported";
		...
		if(!isdefined(level.sm_active_count_title))
		{
			...
			level.sm_active_count_title.alignX = "Dev Block strings are not supported";
		...
		if(!isdefined(level.sm_active_count_min_hud))
		{
			...
			level.sm_active_count_min_hud.alignX = "Dev Block strings are not supported";
		...
		if(!isdefined(level.sm_active_count_max_hud))
		{
			...
			level.sm_active_count_max_hud.alignX = "Dev Block strings are not supported";
		...
		if(!isdefined(level.sm_group_size_min_hud))
		{
			...
			level.sm_group_size_min_hud.alignX = "Dev Block strings are not supported";
		...
		if(!isdefined(level.sm_group_size_max_hud))
		{
			...
			level.sm_group_size_max_hud.alignX = "Dev Block strings are not supported";
		...
		if(!isdefined(level.sm_spawner_count_title))
		{
			...
			level.sm_spawner_count_title.alignX = "Dev Block strings are not supported";
		...
		if(!isdefined(level.sm_spawner_count_min_hud))
		{
			...
			level.sm_spawner_count_min_hud.alignX = "Dev Block strings are not supported";
		...
		if(!isdefined(level.sm_spawner_count_max_hud))
		{
			...
			level.sm_spawner_count_max_hud.alignX = "Dev Block strings are not supported";
		...
		if(increase_value || decrease_value)
		{
			...
			switch(level.current_debug_index)
			{
				...
				case 1:
				{
					...
					{
						modify_debug_hud2("Dev Block strings are not supported");
						break;
					}
		...
		if(isdefined(self))
		{
			level.sm_active_count_min_hud setText("Dev Block strings are not supported" + self.sm_active_count_min);
			level.sm_active_count_max_hud setText("Dev Block strings are not supported" + self.sm_active_count_max);
			level.sm_group_size_min_hud setText("Dev Block strings are not supported" + self.sm_group_size_min);
			level.sm_group_size_max_hud setText("Dev Block strings are not supported" + self.sm_group_size_max);
			if(isdefined(self.sm_spawner_count))
			{
				level.sm_spawner_count_title setText("Dev Block strings are not supported" + self.sm_spawner_count);
				level.sm_spawner_count_min_hud setText("Dev Block strings are not supported" + self.sm_spawner_count_min);
				level.sm_spawner_count_max_hud setText("Dev Block strings are not supported" + self.sm_spawner_count_max);
			}
		}
	#/
}
...
function moniter_debug_hud2()
{
	/#
		self endon("modified");
		wait(10);
		level.spawn_manager_debug_hud2 setText("Dev Block strings are not supported");
	#/
}
